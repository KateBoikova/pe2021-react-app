...
class LoginForm extends Component {
  ...
  handleLoginChange = ({target: {value}}) => {
    this.setState({ loginValue: value, isLoginValid: /^\S+$/.test(value), });
  };
  ...
  render() {
    ...
    // const loginClassName = `${styles.input} ${isLoginValid ? styles.valid : null}`;
    ...
    return (
      <form className={styles.container} onSubmit={this.submitHandler}>
        <label className={styles.inputLabel}>
          {' '}
          Login
          <input className={loginClassName}
          type="text" 
          placeholder= 'login'
          name='loginValue'
          value={loginValue} 
